...
class TestEntrenamiento(TestCase):
    def setUp(self):
        ...
        usuario_login = {"usuario": usuario, "contrasena": contrasena}
    ...
    def test_listar_entrenamientosRutinas(self):
        ...
        for indexEjercicioCreado in self.ejercicios_creados:
            nuevo_entrenamiento = {
                ...
                "rutina": random.randint(1, 3),
            }

            # Se realiza la creación de los entrenamientos
            endpoint_entrenamiento = f"/entrenamientos/{self.usuario_id}"
            headers = {
                "Content-Type": "application/json",
                "Authorization": "Bearer {}".format(self.token),
            }
            resultado_nuevo_entrenamiento = self.client.post(
                endpoint_entrenamiento,
        ...
        # Se realiza la consulta de los entrenamientos de Rutina
        endpoint_entrenamiento = f"/rutinasEntrenamientoPersona/{self.usuario_id}"
        headers = {
            "Content-Type": "application/json",
            "Authorization": "Bearer {}".format(self.token),
        }
